...
function RemoveBgCenterState({
  ...
  if (task?.status === "completed" && task.resultUrl) {
    return (
      <section className="marketing-complete-state watermark-center-state remove-bg-center-state marketing-result-card is-completed">
  ...
  return (
    <section className="marketing-process-state watermark-center-state remove-bg-center-state is-processing" aria-live="polite">
      <span className="watermark-center-spinner">
        <Loader2 size={30} />
      </span>
      <strong>{isSubmitting ? "正在创建抠图任务" : "正在智能抠图"}</strong>
      <p>图片正在处理中，完成后会自动回填到这里。</p>
      <div className="marketing-process-progress watermark-center-progress remove-bg-center-progress">
...
function RemoveBgUploadSlot({ sourceAsset, previewUrl, isUploading, onSelect, onClear }) {
  ...
  return (
    <button className={`marketing-tool-upload watermark-upload-slot remove-bg-upload-slot ${previewUrl ? "has-preview" : ""}`} type="button" onClick={() => inputRef.current?.click()}>
      <input
        ref={inputRef}
        type="file"
        accept="image/*"
...
function RemoveBgComposer({ options, onSubmit, isSubmitting }) {
  ...
  return (
    <div className="marketing-tool-card watermark-composer remove-bg-composer" aria-label="抠图上传面板">
      ...
      <RemoveBgUploadSlot
        ...
        onClear={clearSource}
      />
      <div className="marketing-tool-footer watermark-composer-footer remove-bg-composer-footer">
