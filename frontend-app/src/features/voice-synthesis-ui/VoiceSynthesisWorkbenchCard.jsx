...
import { Clipboard, FolderOpen, HelpCircle, Mic2, SmilePlus, Trash2, Upload, Wand2 } from "lucide-react";
      ...
      <div className="voice-synthesis-workspace__layout">
        <div className="voice-synthesis-workspace__main">
            ...
            <div className="voice-synthesis-workspace__upload-grid">
              <label className={`voice-synthesis-workspace__dropzone ${cloneAudio ? "has-file" : ""}`}>
                ...
                <div className="voice-synthesis-workspace__dropzone-actions">
                  <span><FolderOpen size={16} /> 浏览本地文件</span>
                  {cloneAudio && (
                    <button
                      type="button"
                      onClick={(event) => {
          ...
          <section className="voice-synthesis-workspace__footer">
            <div>
              <strong>状态</strong>
              <p>{notice || "目标音色支持 mp3、m4a、wav，建议时长 10 秒到 5 分钟。"}</p>
            </div>
            <div className="voice-synthesis-workspace__footer-actions">
              ...
              <button type="button" className="is-primary" onClick={onGenerate} disabled={isGenerating || uploading || !cloneAudio || !text.trim()}>
                <Wand2 size={18} />
