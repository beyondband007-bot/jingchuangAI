...
export function MarketingHistoryDetailModal({ task, tool, onClose, onRepeat }) {
  ...
  useEffect(() => {
    if (!task || tool === "replicate" || isVideoTask(task)) {
      setWideMedia(false);
      return undefined;
    }
    const url = task.resultUrl || task.sourceUrl;
    if (!url) return undefined;
    const image = new Image();
    image.onload = () => setWideMedia(image.naturalWidth > image.naturalHeight);
    image.onerror = () => setWideMedia(false);
    image.src = url;
  ...
  return createPortal(
    <div className="marketing-history-detail" role="dialog" aria-modal="true" aria-label={`${tool} 历史详情`}>
      <style>{detailStyles}{wideDetailStyles}</style>
