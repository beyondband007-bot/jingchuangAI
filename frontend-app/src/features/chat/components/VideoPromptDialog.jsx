...
export function VideoPromptDialog({
  ...
  const currentDuration = durationOptions.find((item) => String(item?.value ?? item) === String(duration));
        ...
        <div style={{ display: "flex", alignItems: "center", gap: "8px", flex: "1 1 auto", minWidth: 0, flexWrap: "nowrap" }}>
          ...
          <div className="chatbot-ui-dropdown" style={{ position: "relative" }}>
            <button
              ...
              onMouseLeave={(e) => { e.currentTarget.style.background = "rgba(55, 55, 55, 0.8)"; }}
            >
              <span>{currentDuration?.label || `${duration}s`}</span>
              <svg width="12" height="12" viewBox="0 0 12 12" fill="none" style={{ transform: showDurationDropdown ? "rotate(180deg)" : "rotate(0)", transition: "transform 0.2s ease" }}>
                <path d="M3 4.5L6 7.5L9 4.5" stroke="#888888" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" />
              </svg>
            </button>
              ...
              <div style={dropdownMenuStyle(150)}>
                {durationOptions.map((item) => (
                  <div
                    key={item.value ?? item}
                    onClick={() => {
                      onDurationChange(item.value ?? item);
                      setShowDurationDropdown(false);
                    }}
                    style={{
