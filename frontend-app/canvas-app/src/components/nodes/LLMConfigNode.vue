...
import { TEXT_GENERATION_MODEL } from '../../config/textGeneration'
...
const handleSelect = (item) => {
  ...
  const defaultData = {
    imageConfig: { model: 'gpt-image-2-text-to-image', size: '1x1', label: '文生图' },
...
watch(() => props.data, (newData) => {
  ...
  if (newData?.model !== undefined) model.value = TEXT_GENERATION_MODEL
...
onMounted(() => {
  // 旧项目可能保存过其他文本模型，加载时统一迁移为 DeepSeek V4 Pro。
  if (model.value !== TEXT_GENERATION_MODEL || props.data?.model !== TEXT_GENERATION_MODEL) {
    model.value = TEXT_GENERATION_MODEL
    updateConfig()
  }
...
// 默认模型使用选中的模型
const model = ref(TEXT_GENERATION_MODEL)
// Format options | 格式选项
const formatOptions = [
