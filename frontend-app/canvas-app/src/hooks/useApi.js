  ...
  const send = async (content, stream = true, chatOptions = {}) => {
    ...
    try {
      ...
      if (stream) {
        ...
        const resolvedChatUrl = new URL(chatUrl, window.location.origin)
        const endpoint = `${resolvedChatUrl.pathname}${resolvedChatUrl.search}`

        for await (const chunk of streamChatCompletions(
          adaptedParams,
          abortController.signal,
          { baseUrl: resolvedChatUrl.origin, endpoint, includeAuth: !modelStore.providerConfig.serverManaged }
        )) {
          fullResponse += chunk
