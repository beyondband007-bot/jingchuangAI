...
export const useModelStore = defineStore('model', () => {
  ...
  // 当前选中的渠道
  const currentProvider = ref(getDefaultProvider())
  ...
  const currentApiKey = computed(() => {
    if (providerConfig.value.serverManaged) return 'server-managed'
    return apiKeysByProvider.value[currentProvider.value] || ''
  })
