...
const INPUT_KEYS = new Set(['content', 'prompt', 'model', 'size', 'quality', 'ratio', 'duration', 'systemPrompt', 'outputFormat', 'url'])

export const markDescendantResultsStale = (sourceId) => {
  const queue = [sourceId]
  const visited = new Set()
  while (queue.length) {
    const currentId = queue.shift()
    for (const edge of edges.value.filter(item => item.source === currentId)) {
  ...
  nodes.value = nodes.value.map(node => {
    ...
    const hasResult = (['image', 'video'].includes(node.type) && node.data?.url) ||
      (node.type === 'llmConfig' && node.data?.outputContent)
    return hasResult ? { ...node, data: { ...node.data, inputChanged: true } } : node
  })
}
...
export const getNodeInputHash = (id) => {
  ...
  const incoming = edges.value
    ...
    .map(edge => {
      ...
      return {
        ...
        type: source?.type || '',
        content: source?.data?.content || source?.data?.outputContent || source?.data?.url || ''
      }
    })
...
export const removeNode = (id) => {
  const affectedTargets = edges.value.filter(edge => edge.source === id).map(edge => edge.target)
  nodes.value = nodes.value.filter(node => node.id !== id)
  edges.value = edges.value.filter(edge => edge.source !== id && edge.target !== id)
  affectedTargets.forEach(markDescendantResultsStale)
...
export const addEdge = (params) => {
  ...
  markDescendantResultsStale(params.source)
...
export const removeEdge = (id) => {
  ...
  if (removed) markDescendantResultsStale(removed.target)
