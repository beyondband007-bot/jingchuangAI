...
const exampleImages = [
  ...
  {
    src: '/assets/image/thumb-img-gen.jpg',
    label: '图片生成',
    model: 'Imagen 4 Fast',
    ratio: '1:1',
    quality: '1K',
...
const navSections = [
  ...
  },
  {
    ...
    label: '我的资产',
    icon: Wallet,
    href: 'https://www.getureai.com/portal/index.html',
  },
]
    ...
    >
      ...
      >
        ...
        <h2 id="auth-drawer-title">
          {renderMode === 'register' ? '创建账号' : '欢迎回来'}
        </h2>
        <p>
          {renderMode === 'register'
            ? '注册后立即获得 1000 积分，开始保存你的生成记录。'
            : '登录后即可查看你的积分与生成记录。'}
        </p>
        <form className="auth-form" onSubmit={submit}>
          ...
          <label>
            ...
            <input
              ...
              type="password"
              autoComplete={
                renderMode === 'register' ? 'new-password' : 'current-password'
              }
            />
          </label>
          {renderMode !== 'register' && (
...
function ResultCard({
  ...
  return (
    <article
      className={`result-card status-${card.status} ${isExample ? 'is-example' : ''} ${isImageGallery ? 'is-image-gallery' : ''}`}
    >
      {isImageGallery && (
      ...
      <div className="chat-conversation-thread">
        {messages.map((message) => (
          <div className={`chat-message-row ${message.role}`} key={message.id}>
            ...
            <div
              className={`chat-message-bubble ${message.status === 'failed' ? 'is-error' : ''}`}
            >
              {message.status === 'failed' ? (
                <>
...
function ChatComposerBar({ options, onSubmit, isSubmitting }) {
  ...
  const [model, setModel] = useState(
    options.defaultModel || options.models[0]?.value || '',
  )
  const [reasoningEffort, setReasoningEffort] = useState(
    options.reasoningEfforts[0]?.value || 'none',
  )
  const [notice, setNotice] = useState('')
      ...
      <div className="llm-toolbar">
        <div className="llm-left">
          <button
            ...
            type="button"
            onClick={() => setNotice('上传按钮暂未接入文件选择器。')}
            aria-label="上传"
          >
            +
          </button>
          <div
            className={`llm-select-wrap ${openMenu === 'inspiration' ? 'is-open' : ''}`}
          >
            <button
              ...
              onClick={() =>
                setOpenMenu((current) =>
                  current === 'inspiration' ? null : 'inspiration',
                )
              }
            >
        ...
        <div className="llm-right">
          ...
          <button
            className="llm-round"
            type="button"
            onClick={() => setNotice('录音按钮暂未接入麦克风权限。')}
            aria-label="录音"
          >
            <Mic size={18} />
          </button>
...
function LegacyChatGenerationView() {
  ...
  const isIntroState =
    !messages.length && !isSubmitting && !submitError && !isHistoryOpen
        ...
        <div className="dh-selected-template-media">
          {selectedAvatar?.cover ? (
            selectedAvatarIsVideo ? (
              ...
              <img src={selectedAvatar.cover} alt={selectedAvatar.name} />
            )
          ) : (
            <UserRound size={22} />
          )}
      ...
      <div className="dh-settings-group">
        <div className="dh-minimax-options">
          ...
          <label className="dh-field">
            ...
            <select
              value={ttsEmotion}
              onChange={(event) => setTtsEmotion(event.target.value)}
            >
              {ttsEmotionOptions.map((item) => (
...
function DigitalHumanGenerationView() {
  ...
  const visibleAvatars =
    tab === 'mine' ? avatars.mine : getDigitalHumanPublicAvatars(avatars.public)
                ...
                <DigitalHumanEmptyMedia
                  ...
                  description={
                    currentPreviewTask?.status === 'failed'
                      ...
                      : currentPreviewTask
                        ? `任务已提交，MiniMax 生成音频后生成口型视频。当前进度 ${currentPreviewTask.progress || 0}%`
                        : '这里会展示选中形象或生成后的视频'
                  }
                />
              )}
    ...
    <article className={`idh-result-card status-${task.status}`}>
      <div className="idh-result-preview">
        {hasVideo ? (
          <>
            ...
            />
            <button
              className="idh-fullscreen-button"
              type="button"
      ...
      <div className="idh-result-meta">
        <div className="idh-result-title">
          ...
          <span>
            {formatProviderLabel(task.usedProviderModel || task.providerModel)}
          </span>
        </div>
        ...
        <div className="idh-result-actions">
          <small>
            {task.time} · {task.costPoints || 0} 积分
          </small>
          <span>
            {task.resultUrl && (
              <a
...
function ImageDigitalHumanCenterState({
  task,
  isSubmitting,
  error,
  onOpenRecent,
  ...
  const isProcessing =
    isSubmitting || task?.status === 'processing' || task?.status === 'pending'
    ...
    return (
      <section className="idh-center-state is-completed" aria-live="polite">
        <div className="idh-center-copy">
          ...
          <strong>视频已生成</strong>
          <p>
            本次图片数字人视频已保存到最近生成。首页不再展示案例，方便继续创建下一条视频。
          </p>
  ...
  return (
    <section className="idh-center-state is-processing" aria-live="polite">
      ...
      <p>
        正在生成语音、上传人物照片，并合成口型视频。完成后会自动回填到这里。
      </p>
...
function ImageDigitalHumanComposer({
  options,
  voices,
  onSubmit,
  isSubmitting,
  ...
  const selectedModel =
    options.models.find((item) => item.value === model) || options.models[0]
  const selectedVoice = voices.find((item) => item.id === voiceId) || voices[0]
  const previewSignature = getDigitalHumanPreviewSignature({
    text,
    voiceId,
    speed,
    volume,
  ...
  function submit() {
    ...
    if (isTooLong) {
      setNotice(
        `当前音频 ${formatDurationMs(voicePreviewInfo.durationMs)}，超过 15 秒`,
      )
      return
    }
    setNotice('')
  ...
  return (
    <div className="idh-composer" aria-label="图片数字人生成器">
      ...
      <div className="idh-composer-body">
        <button
          className={`idh-upload-card ${portraitPreview ? 'has-image' : ''}`}
          type="button"
          onClick={() => fileInputRef.current?.click()}
        >
          <input
      ...
      <div className="idh-composer-footer">
        ...
        <small>
          {selectedVoice?.description || selectedModel?.primaryModel}
        </small>
        <strong>{price}</strong>
        <button
          type="button"
          onClick={submit}
          disabled={isSubmitting || !isPreviewCurrent || isTooLong}
          aria-label="生成图片数字人视频"
        >
          {isSubmitting ? <Loader2 size={18} /> : <Send size={18} />}
        </button>
      </div>
      {notice && (
        <div className={`idh-notice ${isTooLong ? 'is-warning' : ''}`}>
          {notice}
        </div>
      )}
    </div>
...
function ImageDigitalHumanView() {
  ...
  const visibleResultTasks =
    viewTab === 'recent' && submittedTaskId
      ? tasks.filter((task) => String(task.id) !== String(submittedTaskId))
      : viewTab === 'recent'
        ? tasks
        : []
  const showRecentEmpty =
    viewTab === 'recent' && !showCenterState && visibleResultTasks.length === 0
  const selectedModel =
    options.models.find((item) => item.value === model) || options.models[0]
  const selectedVoice = voices.find((item) => item.id === voiceId) || voices[0]
    ...
    <div className="motion-composer" aria-label={copy.panelLabel}>
      ...
      <div className="motion-composer-footer">
        ...
        <label className="control-select">
          ...
          >
            {(options.modes || emptyMotionTransferOptions.modes).map((item) => (
              <option key={item.value} value={item.value}>
                {item.label}
              </option>
            ))}
        ...
        <label className="control-select">
          ...
          <select
            ...
            onChange={(event) => setCharacterOrientation(event.target.value)}
          >
            {(
...
function App() {
  ...
  useEffect(() => {
    ...
    authApi
      ...
      .catch(() => {
        if (mounted)
          setAuthUser({ isGuest: true, displayName: '游客', username: 'guest' })
      })
    return () => {
      mounted = false
    }
  ...
  const page = (() => {
    ...
    return (
      <SplashHome onOpenAuth={setAuthDrawerMode} onGuestEnter={enterAsGuest} />
    )
  })()
